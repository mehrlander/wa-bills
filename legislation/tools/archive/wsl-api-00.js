...
// fetchAndParse is now a neutral tool - it returns everything it finds
...
// fetchRaw remains for complex nested objects
...
/** BASE FETCHERS - Filtering for 'B' (Bills) is applied here locally **/
...
export const getLegislation = async (sinceDate) => {
    ...
    const arr = findArray(parsed) || [parsed];
    return arr
        .filter(item => (item.ShortLegislationType || item.LegislationInfo?.ShortLegislationType) === 'B')
        .map(transform)
        .filter(Boolean);
};
