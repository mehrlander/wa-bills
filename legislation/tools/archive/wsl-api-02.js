...
export const getLegislation = async (sinceDate) => {
    ...
    return arr
        .filter(item => (item.ShortLegislationType?.ShortLegislationType || item.LegislationInfo?.ShortLegislationType?.ShortLegislationType) === 'B')
        .map(transform)
        .filter(Boolean);
};
...
export const getHistoryFor = async (b, biennium = '2025-26', beginDate = '1/1/2025', endDate = '12/31/2026') => {
    ...
    return (p.ArrayOfLegislativeStatus?.LegislativeStatus || []).map(s => ({
        BillNumber: bn, BillId: s.BillId, ActionDate: sanitize(s.ActionDate), HistoryLine: s.HistoryLine, Status: s.Status, AmendmentsExist: s.AmendmentsExist ? '1' : '0'
    }));
};
...
export const joinRcw = (bill, rcwsMap) => {
    ...
    if (rcw) {
        bill['r.Rcws'] = rcw.Rcws; bill['r.PensionLabels'] = rcw.PensionLabels; bill['r.AdjacentLabels'] = rcw.AdjacentLabels; bill['r.PensionRcws'] = rcw.PensionRcws; bill['r.AdjacentRcws'] = rcw.AdjacentRcws; bill['r.isPension'] = rcw.isPension;
    }
    return bill;
};

export const joinActions = (bill, actionsMap) => {
    const bn = getBillNumber(bill); const acts = actionsMap[bn];
    if (acts?.length) {
        const last = acts[acts.length - 1]; bill['a.Count'] = String(acts.length); bill['a.LastDate'] = last.HearingDate; bill['a.LastCommittee'] = last.Committee; bill['a.LastMajorityCode'] = last['r.MajorityCode'] || ''; bill['a.LastMinorityCode'] = last['r.MinorityCode'] || ''; bill['a.Committees'] = [...new Set(acts.map(a => a.Committee))].join('|');
    }
    return bill;
};

export const joinHistory = (bill, historyMap) => {
    const bn = getBillNumber(bill); const hist = historyMap[bn];
    if (hist?.length) {
        const last = hist[hist.length - 1]; bill['h.Count'] = String(hist.length); bill['h.FirstDate'] = hist[0].ActionDate; bill['h.LastDate'] = last.ActionDate; bill['h.LastLine'] = last.HistoryLine; bill['h.Status'] = last.Status;
    }
    return bill;
};
...
export const joinSponsors = (bill, sponsorsMap) => {
    ...
    if (sp) { bill['ps.LongName'] = sp.LongName; bill['ps.Agency'] = sp.Agency; bill['ps.Acronym'] = sp.Acronym; }
    return bill;
};
