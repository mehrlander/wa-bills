...
const categorize = (s='', a='') => {
    ...
    if (sl.includes('prefiled')) return ch + 'PREFILED';
...
<div id="hdr" style="padding:4px; font-family:sans-serif; border-bottom:1px solid #ddd; display:flex; gap:6px; align-items:center; background:#f4f4f4; position:sticky; top:0; z-index:100;">
  ...
  <button id="b" style="${tBtn} background:#e1e1e1;">Load All Bills</button>
...
$('#b').on('click',async()=>{
$('#s').text('Fetching data...'); $('#act').hide();
let dVal=new Date($('#d').val()),
dStr = `${dVal.getMonth()+1}/${dVal.getDate()}/${dVal.getFullYear()}`,
urls = [
    `/LegislationService.asmx/GetLegislationIntroducedSince?sinceDate=${dStr}`,
    `/LegislationService.asmx/GetPrefiledLegislation`
];

try {
    const resps = await Promise.all(urls.map(u => fetch(u).then(r => r.text())));
    const rawAll = resps.flatMap(xml => findArray(parser.parse(xml)) || []);
    
    // Deduplicate and process
    const seen = new Set();
    const v=t=>t===true?1:t===false?0:typeof t==='string'?(t.startsWith('0001')?null:t.trim().replace(/T00:00:00$/,'')):t;
    const short=s=>s.replace(/^CurrentStatus\./,'cs.').replace(/^ShortLegislationType\./,'slt.').replace(/^Companions\./,'comp.').replace(/^ActionDate$/,'cs.ActionDate');
    
    const items = rawAll.filter(r => seen.has(r.BillId) ? false : seen.add(r.BillId)).map(o => 
        Object.fromEntries(Object.entries(flatten(o)).map(([k,val])=>[short(k),v(val)]))
    );
    
    const k=[...new Set(items.flatMap(Object.keys))].sort();
    ...
    const tab2 = new Tabulator('#t2',{
      ...
      columns:[
        ...
        {title:'Status/Action', field:'status', maxWidth:1000, formatter:c=>{
            ...
            else if (cat.includes('PREFILED')) { color = '#6c757d'; text = 'white'; border = '#495057'; }
