...
const PENSION_MAP = {
    ...
    adjacent: { "6.15": "Pension exemptions", "26.16": "Marital property", "26.18": "Support access", "41.28": "Local fire", "41.44": "Local city", "51.32": "Workers Comp Disability", "74.20A": "DCS enforcement" },
    special: { "41.40.124": "Judicial Multiplier", "41.40.761": "Judicial Multiplier P2", "41.45.0631": "WSPRS Rates", "41.50.770": "DCP", "41.50.780": "DCP Accounts", "41.26.130": "Disability Offset", "41.26.470": "Disability Offset", "41.37.230": "Disability Offset", "41.40.038": "L&I Service Credit", "41.37.060": "L&I Service Credit", "41.26.473": "L&I Service Credit", "41.26.048": "Line of Duty Death" }
};
...
export const classifyPensionBill = (rcwList = []) => {
    ...
    rcwList.forEach(cite => {
        const parts = cite.split('.'), ch = parts.slice(0, 2).join('.'), sec = parts[2] || null;
        findInMap(ch, sec).forEach(m => {
            if (['system', 'general', 'governance'].includes(m.cat)) { pension.push(m); rcwsP.push(cite); }
            else if (m.cat === 'adjacent') { adjacent.push(m.label); rcwsA.push(cite); }
        });
    });
...
export const transform = r => {
    ...
    for (let k in f) {
        ...
        Object.entries(ABBR).forEach(([long, short]) => { if (key.startsWith(long)) key = key.replace(long, short); });
        out[key] = val;
    }
    return out;
};
